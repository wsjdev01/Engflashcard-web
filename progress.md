...
- **다음 세션 시작 지점**:
  1. YouTube 자막 자동화 기본 흐름은 이제 정상 동작함. 다음 좋아요 영상 처리하려면 그냥 `python3 youtube_to_sheets.py`(터미널에서, `.env`에 키 있으니 재입력 불필요) 실행하면 됨
  2. 아직 안 한 것: **주기 자동 실행 등록**(`schedule` 스킬 등으로 매일 1회 자동화) — 지금은 사람이 터미널에서 수동 실행해야 함, 원래 최종 목표(진짜 "에이전트"화)는 아직 안 이루어짐
  3. `clean_sentences()`의 반복 캡션 병합 로직 엣지 케이스(16행 사례) 근본 수정은 미룸 — 다음에 비슷한 사례 나오면 그때 로직 자체를 고칠 것
